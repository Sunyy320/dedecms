...
class DataListCP
{
    ...
    function GetArcList($atts,$refObj='',$fields=array())
    {
        ...
        {
            $i++;
            //添加下面俩行代码就可以把arcurl放入到已经查询的数组当中
            $get_one=GetOneArchive($arr['id']);
            $arr['arcurl']=$get_one['arcurl'];
...
class plDataListCp extends  DataListCP{
    //获取分页导航列表
    function GetPageList($atts,$refObj='',$fields=array())
    {
        global $lang_pre_page,$lang_next_page,$lang_index_page,$lang_end_page,$lang_record_number,$lang_page,$lang_total;
...
//            $atts['listitem'] = "info,index,end,pre,next,pageno";
        ...
        if($this->pageNO != 1)
        {
            $prepage .= "<a class='prePage' href='".$purl."pageno=$prepagenum'>上页</a> \n";
            $indexpage = "<a class='indexPage' href='".$purl."pageno=1'>首页</a> \n";
        }
        else
        {
//            $indexpage = "<span class='indexPage'>"."$lang_index_page \n"."</span>";
        ...
        if($this->pageNO != $totalpage && $totalpage > 1)
        {
            $nextpage.="<a class='nextPage' href='".$purl."pageno=$nextpagenum'>下页</a> \n";
            $endpage="<a class='endPage' href='".$purl."pageno=$totalpage'>末页</a> \n";
        }
        else
        {
//            $endpage=" <strong>$lang_end_page</strong> \n";
